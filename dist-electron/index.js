...
function requireUuid() {
  ...
  class UUID {
    constructor(uuid2) {
      this.ascii = null;
      this.binary = null;
      const check2 = UUID.check(uuid2);
      if (!check2) {
        throw new Error("not a UUID");
      }
      this.version = check2.version;
      if (check2.format === "ascii") {
        this.ascii = uuid2;
      } else {
        this.binary = uuid2;
      }
...
function getStats$2(src2, dest, opts2) {
...
function checkPaths(src2, dest, funcName, opts2, cb) {
  util$1.callbackify(getStats$2)(src2, dest, opts2, (err, stats) => {
    if (err)
      return cb(err);
...
const stat$3 = stat$4;
function copy$2(src2, dest, opts2, cb) {
  ...
  stat$3.checkPaths(src2, dest, "copy", opts2, (err, stats) => {
    if (err)
      return cb(err);
    const { srcStat, destStat } = stats;
    stat$3.checkParentPaths(src2, srcStat, dest, "copy", (err2) => {
      if (err2)
        return cb(err2);
      if (opts2.filter)
        return handleFilter(checkParentDir, destStat, src2, dest, opts2, cb);
      return checkParentDir(destStat, src2, dest, opts2, cb);
    });
  });
}
function checkParentDir(destStat, src2, dest, opts2, cb) {
  const destParent = path$g.dirname(dest);
  pathExists$5(destParent, (err, dirExists) => {
    if (err)
      return cb(err);
    if (dirExists)
      return getStats$1(destStat, src2, dest, opts2, cb);
...
function handleFilter(onInclude, destStat, src2, dest, opts2, cb) {
  Promise.resolve(opts2.filter(src2, dest)).then((include) => {
    if (include)
      return onInclude(destStat, src2, dest, opts2, cb);
    return cb();
  }, (error) => cb(error));
}
function startCopy$1(destStat, src2, dest, opts2, cb) {
  if (opts2.filter)
    return handleFilter(getStats$1, destStat, src2, dest, opts2, cb);
  return getStats$1(destStat, src2, dest, opts2, cb);
}
function getStats$1(destStat, src2, dest, opts2, cb) {
  const stat2 = opts2.dereference ? fs$c.stat : fs$c.lstat;
  stat2(src2, (err, srcStat) => {
    if (err)
      return cb(err);
    if (srcStat.isDirectory())
      return onDir$1(srcStat, destStat, src2, dest, opts2, cb);
    else if (srcStat.isFile() || srcStat.isCharacterDevice() || srcStat.isBlockDevice())
      return onFile$1(srcStat, destStat, src2, dest, opts2, cb);
    else if (srcStat.isSymbolicLink())
      return onLink$1(destStat, src2, dest, opts2, cb);
...
function onFile$1(srcStat, destStat, src2, dest, opts2, cb) {
  if (!destStat)
    return copyFile$1(srcStat, src2, dest, opts2, cb);
  return mayCopyFile$1(srcStat, src2, dest, opts2, cb);
}
function mayCopyFile$1(srcStat, src2, dest, opts2, cb) {
  if (opts2.overwrite) {
...
function copyFile$1(srcStat, src2, dest, opts2, cb) {
...
function onDir$1(srcStat, destStat, src2, dest, opts2, cb) {
  if (!destStat)
    return mkDirAndCopy$1(srcStat.mode, src2, dest, opts2, cb);
  return copyDir$1(src2, dest, opts2, cb);
}
function mkDirAndCopy$1(srcMode, src2, dest, opts2, cb) {
  fs$c.mkdir(dest, (err) => {
    if (err)
      return cb(err);
    copyDir$1(src2, dest, opts2, (err2) => {
      if (err2)
        return cb(err2);
      return setDestMode$1(dest, srcMode, cb);
    });
...
function copyDir$1(src2, dest, opts2, cb) {
  fs$c.readdir(src2, (err, items) => {
    if (err)
      return cb(err);
    return copyDirItems(items, src2, dest, opts2, cb);
  });
}
function copyDirItems(items, src2, dest, opts2, cb) {
  const item = items.pop();
  if (!item)
    return cb();
  return copyDirItem$1(items, item, src2, dest, opts2, cb);
}
function copyDirItem$1(items, item, src2, dest, opts2, cb) {
...
function onLink$1(destStat, src2, dest, opts2, cb) {
...
const stat$2 = stat$4;
function copySync$1(src2, dest, opts2) {
  ...
  const { srcStat, destStat } = stat$2.checkPathsSync(src2, dest, "copy", opts2);
  stat$2.checkParentPathsSync(src2, srcStat, dest, "copy");
  return handleFilterAndCopy(destStat, src2, dest, opts2);
}
function handleFilterAndCopy(destStat, src2, dest, opts2) {
  if (opts2.filter && !opts2.filter(src2, dest))
    return;
...
function startCopy(destStat, src2, dest, opts2) {
  if (opts2.filter && !opts2.filter(src2, dest))
    return;
  return getStats(destStat, src2, dest, opts2);
}
function getStats(destStat, src2, dest, opts2) {
  ...
  if (srcStat.isDirectory())
    return onDir(srcStat, destStat, src2, dest, opts2);
  else if (srcStat.isFile() || srcStat.isCharacterDevice() || srcStat.isBlockDevice())
    return onFile(srcStat, destStat, src2, dest, opts2);
  else if (srcStat.isSymbolicLink())
    return onLink(destStat, src2, dest, opts2);
  else if (srcStat.isSocket())
...
function onDir(srcStat, destStat, src2, dest, opts2) {
  if (!destStat)
    return mkDirAndCopy(srcStat.mode, src2, dest, opts2);
...
function copyDir(src2, dest, opts2) {
  fs$b.readdirSync(src2).forEach((item) => copyDirItem(item, src2, dest, opts2));
}
...
const stat$1 = stat$4;
function move$1(src2, dest, opts2, cb) {
  if (typeof opts2 === "function") {
    cb = opts2;
    opts2 = {};
  }
  ...
  stat$1.checkPaths(src2, dest, "move", opts2, (err, stats) => {
    if (err)
      return cb(err);
...
function getAppCacheDir() {
  ...
  if (process.platform === "win32") {
    ...
    result = path$4.join(homedir, "Library", "Application Support", "Caches");
  } else {
...
function requireAppUpdater() {
  ...
  let AppUpdater$1 = class AppUpdater2 extends events_12.EventEmitter {
    ...
    async executeDownload(taskOptions) {
      ...
      const tempUpdateFile = await DownloadedUpdateHelper_1.createTempUpdateFile(`temp-${updateFileName}`, cacheDir, log2);
      try {
