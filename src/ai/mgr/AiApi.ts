...
export const AiApi = {
  ...
  getAiChatListByBotId : async function(botId:number,success:(response:any)=>void) {
    const rpc = new BaseRpcRequest(baseUrl + `/chat/list?botId=${botId}`,success)
    return rpc.send()
  },
  ...
  generateTitle : async (data:IAiChatGenerateDef, success:(response:any)=>void)=> {
    const rpc = new BaseRpcRequest(baseUrl + "/chat/completions",success,"POST",data)
    return rpc.send()
  },
  ...
  {
    const streamRpc = new StreamRpcRequest(baseUrl + `/chat/completions`,success,"POST",data,undefined,onEnd)
    streamRpc.send()
  },
  addMessage : async function(data:IAiChatMessageDef, success:(response:any)=>void)
  {
    const rpc = new BaseRpcRequest(baseUrl + "/message",success,"POST",data)
    return rpc.send()
  },
  setTemplate: async function(data:IAiBotUpdateDef, success:(response:any)=>void)
  {
    const rpc = new BaseRpcRequest(baseUrl + "/bot/template",success,"PUT",data)
    return rpc.send()
  }
}